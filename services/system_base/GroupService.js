/**
 * Groups 相關之商業邏輯
 * @module services/system_base/GroupService.js
 */
"use strict";
...
module.exports.insertGroup = async (req, res, next) => {
	...
	try
	{
		...
		await groupRepository.createGroup({
			"Name":         req.body.data.group_name,
			"Description":  req.body.data.group_description,
			"Role":         req.body.data.group_role,
		});
...
module.exports.deleteGroup = async (req, res, next) => {
	...
	try
	{
		...
		// check group existed
		const isExisted = await groupRepository.isGroupsExisted({"Id" : req.body.data.group_id});
		if(isExisted === false) throw(new Error("ERROR_NOT_EXISTED_DATA"));

		// delete data
		await groupRepository.destroyGroup({"Id" : req.body.data.group_id});
...
module.exports.updateGroup = async (req, res, next) => {
	...
	try
	{
		...
		await groupRepository.setGroup(
			...
			{ "Id": decodeURIComponent(req.body.data.group_id) });
